...
class Action {
  ...
  async plugin(plugin) {
    switch (plugin) {
      case "checkout":
        console.log(this.run.event.repository);
        let repo =
  ...
  async execute() {
    if (this.type === "repo") {
      ...
      var actionConfig = loadYaml.load(`${this.path}/action.yml`);
    ...
    if (this.step.uses && actionConfig.runs.using === "docker") {
      this.stack.push(
    ...
    if (this.step.uses && actionConfig.runs.using === 'node12') {
      this.stack.push(`node ${this.path}/${actionConfig.runs.main}`);
    }

    if (this.step.run) {
      this.stack.push(this.step.run);
    }
