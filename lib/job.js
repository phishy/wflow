  ...
  asyncForEach(job.steps, async step => {
    ...
    step.syslog.server.on("message", data => {
      ...
      fs.appendFile(`${logPath}/${stepName}.log`, msg, err => {
        if (err) throw err;
        if (step.syslog.broadcasting) return;
    ...
    if (step.uses) {
      ...
      stack.push(
        `docker run --log-driver syslog --log-opt syslog-address=udp://host.docker.internal:${step.syslog.port} -v ${codePath}:/code ${envStr} ${dockerInDocker} ${entrypoint} ${action} ${args}`
      );
      try {
        await execa.command(stack.join(";"), { shell: true });
    ...
    if (step.run) {
      let cmd = `docker run --name ${stepName} --log-driver syslog --log-opt syslog-address=udp://host.docker.internal:${step.syslog.port} --rm -v ${codePath}:${codePath} ${envStr} ${dockerInDocker} ${image} bash -c 'cd ${codePath}; ${step.run}'`;
      logger.debug(cmd);
