...
async function run(workflow, secrets, job, ports) {
  ...
  asyncForEach(job.steps, async step => {
    ...
    if (step.uses) {
      ...
      stack.push(
        `docker run -l wflow --log-driver syslog --log-opt syslog-address=udp://${internalIp.v4.sync()}:${step.syslog.port} -v ${codePath}:/code ${envStr} ${dockerInDocker} ${entrypoint} ${action} ${args}`
      );
      try {
        await axios.patch(`http://localhost:3000/steps/${step._id}`, {
          status: "incomplete"
    ...
    if (step.run) {
      let cmd = `docker run -l wflow --name ${stepName} --log-driver syslog --log-opt syslog-address=udp://${internalIp.v4.sync()}:${step.syslog.port} --rm -v ${codePath}:${codePath} ${envStr} ${dockerInDocker} ${image} bash -c 'cd ${codePath}; ${step.run}'`;
      logger.debug(cmd);
